...
public class Aquifers {
    ...
    public static Mesh aquifers(Mesh mesh, int numAquifers, String[] args) {
        ...
        for (int i = 0; i < numAquifers; i++) {
            ...
            if (!selectedPolygon.getProperties(1).getValue().equals("land") && !selectedPolygon.getProperties(2).getValue().equals("0")) {
                ...
                for (int j = 0; j < neighbours.size(); j++) {
                    ...
                    if (!neighbour.getProperties(1).getValue().equals("ocean") && !neighbour.getProperties(1).getValue().equals("lake") && !neighbour.getProperties(1).getValue().equals("aquifer")) {
                        count += 1;
                    }
                }
                if (count == neighbours.size()) {
                    if(config.soil()==null){
                        String color = 36 + "," + 100 + "," + 7;
                        Property aquifer = Property.newBuilder().setKey("type").setValue("aquifer").build();
                        Property fill = Property.newBuilder().setKey("rgb_color").setValue(color).build();
                        Polygon newPolygon = Polygon.newBuilder(selectedPolygon).setProperties(0, fill).setProperties(1,aquifer).build();
                        polygon_List.set(x,newPolygon);
                    }
                    else if (config.soil().equals("dry")){
                        String color = 34 + "," + 139 + "," + 34;
                        Property aquifer = Property.newBuilder().setKey("type").setValue("aquifer").build();
                        Property fill = Property.newBuilder().setKey("rgb_color").setValue(color).build();
                        Polygon newPolygon = Polygon.newBuilder(selectedPolygon).setProperties(0, fill).setProperties(1,aquifer).build();
                        polygon_List.set(x,newPolygon);
                    ...
                    for (int j = 0; j < neighbours.size(); j++) {
                        Polygon neighbour = polygon_List.get(neighbours.get(j));
                        if (!neighbour.getProperties(1).getValue().equals("lake") && !neighbour.getProperties(1).getValue().equals("aquifer") && !neighbour.getProperties(1).getValue().equals("beach") && neighbour.getProperties(2).getValue().equals("0") ) {
                            if(config.soil()==null){
